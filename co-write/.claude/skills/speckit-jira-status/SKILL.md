...
description: Read-only sync status / drift preview — disk vs Jira; runs reconcile
  in --dry-run and never mutates
compatibility: Requires spec-kit project structure with .specify/ directory
metadata:
  author: github-spec-kit
  source: jira:commands/jira-status.md
---
...
Report sync status / drift between the consumer repo's filesystem spec state and
Jira. This is **READ-ONLY**: it runs the reconcile engine in `--dry-run`, which
plans every write but issues **none**, so it mutates neither Jira nor the
filesystem. Execute it yourself via the Bash tool and summarize; do NOT ask the
user to run anything in their terminal.
